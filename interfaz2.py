...
st.title('🌦 Clasificación Climática con Random Forest')

# Imagen ilustrativa
try:
    image = Image.open("clima1.jpeg")
    st.image(image, caption="Condiciones climáticas representativas", use_container_width=True)
except:
    st.warning("No se pudo cargar la imagen 'clima1.jpeg'. Asegúrate de que esté en la misma carpeta.")

# Ingreso manual de variables
st.header("🔢 Ingrese los valores para la predicción")
...
Precipitation = st.slider("Precipitación (%)", 0, 100, 50)
...
UV_Index = st.slider("Índice UV", 0, 14, 5)
Season = st.selectbox("Estación", season_options)
Visibility = st.number_input("Visibilidad (km)", value=5.0)
Location = st.selectbox("Ubicación", location_options)

# Cargar dataset
df = pd.read_csv("weather_classification_data.csv")  # Asegúrate de tener este archivo en la misma carpeta

# Sidebar para mostrar datos
st.sidebar.header("📂 Fuente de datos")
if st.sidebar.checkbox("Mostrar muestra del dataset"):
    num_rows = st.sidebar.slider("Número de filas a mostrar", 1, 20, 5)
...
# Sidebar de hiperparámetros
st.sidebar.header("⚙️ Hiperparámetros del modelo")
st.sidebar.markdown("""
- n_estimators: 100
- max_depth: 10
- min_samples_split: 5
...
# Botón de predicción
if st.button("🌤️ Predecir tipo de clima"):
    ...
    st.subheader(f"🌤️ Clima predicho: **{prediction_label}**")
    ...
    try:
        ...
        st.warning(f"No se encontró imagen para '{prediction_label}.jpg'.")
